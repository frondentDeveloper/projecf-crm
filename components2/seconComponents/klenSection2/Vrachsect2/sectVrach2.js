...
function VrachK(props) {
    ...
    function getClentUserV() {
        ...
        axios.get("https://human1moment.herokuapp.com/api/client/?job_type=Vrach").then((response) => {
            setGetClentsV(response.data.data);
            console.log(response.data)
            setLoading(false)
        })
    }
    const [getClentsV, setGetClentsV] = useState([]);
    ...
    function putitem() {
        axios.put(`https://human1moment.herokuapp.com/api/client/$id/`, getClentsV).then((response)=>{
            getClentUserV()
            console.log(response.data)
        })
    }

    const [seracgTerm, setSearchTerm] = useState("")
                    ...
                    <Content className="site-layout-background" style={{ margin: '24px 16px 0', overflow: 'initial' }}>
                        ...
                        <div className="for-table">
                            <Table bordered hover>
                                ...
                                {getClentsV.length ? ( <tbody className="body-table">
                                {
                                    getClentsV && getClentsV.filter((value => {
                                        if (seracgTerm === ""){
                                         return value
                                        }else if (value.first_name.toLowerCase().includes(seracgTerm.toLowerCase())){
                                            return value
                                        }
                                    })).map((item, index)=>(
                                        <tr>
