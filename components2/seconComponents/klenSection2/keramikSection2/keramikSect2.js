...
import '/yangi/poytaxt-crm/src/Admin/components2/klentPage/klent.css'
import {
    SearchOutlined,
    PlusCircleOutlined,
...
function KeramikJs(props) {
    ...
    function getClentUser() {
        ...
        axios.get("https://human1moment.herokuapp.com/api/client/?job_type=Keramis").then((response) => {
            setGetClents(response.data.data);
    ...
    const [getClents, setGetClents] = useState([]);
    ...
    function dleteItem(id) {
        setLoading(true);
        axios.delete(`https://human1moment.herokuapp.com/api/client/${id}/`,{
            headers:{
    ...
    function UpdetaItem(id) {
        axios.put(`https://human1moment.herokuapp.com/api/client/${id}/`, getClents, {
            headers:{
                "Authorization":localStorage.getItem("token")
            }
        }).then(response => {
                    ...
                    <Content className="site-layout-background" style={{margin: '24px 16px 0', overflow: 'initial'}}>
                        ...
                        <div className="for-table">
                            <Table bordered hover>
                                ...
                                {getClents.length ? (<tbody className="body-table">
                                {
                                    getClents.filter((value => {
                                        if (searchTerm === ""){
                                            return value
